...
if( !function_exists('houzez_compose_send_email') ):
    function houzez_compose_send_email($args){
        ...
        if($prop_selection->have_posts()){
            ...
            $mail_content .= '<div style="padding: 0; margin: 0;">';
            $mail_content .= '<div class="main-wrap" style="text-align: center; font-family: Arial, sans-serif; font-size: 14px; line-height: 22px;>';
            while ($prop_selection->have_posts()): $prop_selection->the_post();
                $post_id = get_the_id();
                ...
                if( has_post_thumbnail( $post->ID ) ) {
                    $thumbnail = get_the_post_thumbnail( 'houzez-property-thumb-image' );
                }

                $mail_content .= '<div class="property-wrap" style="text-aling:center; background: #FFFFFF; max-width: 405px; margin: 0 auto 20px; padding: 10px 10px 30px;">
                <a href="'.get_permalink().'" style="display: block;">
                    <img style="width: 100%; height: auto;" src="'.$thumbnail.'">
                </a>
                <h2 style="font-size: 16px; line-height: 24px; margin-bottom: 0;">'.houzez_taxonomy_simple('property_status').' '.$price_prefix.houzez_get_property_price($sale_price).'</h2>
                <p style="margin: 0 0 10px; color: #777777;">'.get_the_title().'</p>

                <p style="margin: 0; font-weight: bold;">'.$beds.' - '.$baths.' - '.$area.'</p>
                <p style="margin: 0 0 10px; font-weight: bold;">'.houzez_taxonomy_simple('property_type').'</p>

                <div style="margin: 20px 0 0;">
                    <a style="background: #00AEEF; border-radius: 4px; color: #ffffff; text-decoration: none; padding: 10px 20px; display: inline-block;" href="'.get_permalink().'">'.esc_html__('View Property', 'houzez').'</a>
                </div>
            </div>';
